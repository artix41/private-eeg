...
class DataLoader:
    ...
    def load_mnist_iid(self):
        ...
        n_samples_train_per_worker = len(mnist_train.targets) // self.n_clients
        n_samples_test_per_worker = len(mnist_test.targets) // self.n_clients

        self.X_train_ptr_client = [
            transform_mnist(
                mnist_train.data[n_samples_train_per_worker * i:n_samples_train_per_worker * (i + 1)]
            ).send(self.clients[i]) for i in range(self.n_clients)]

        self.Y_train_ptr_client = [
            mnist_train.targets[n_samples_train_per_worker * i:n_samples_train_per_worker * (i + 1)].send(self.clients[i])
            for i in range(self.n_clients)]

        self.X_test_ptr_client = [
            transform_mnist(
                mnist_test.data[n_samples_test_per_worker * i:n_samples_test_per_worker * (i + 1)]
            ).send(self.clients[i]) for i in range(self.n_clients)]

        self.Y_test_ptr_client = [
            mnist_test.targets[n_samples_test_per_worker * i:n_samples_test_per_worker * (i + 1)].send(self.clients[i])
            for i in range(self.n_clients)]

        self.n_samples_train = len(self.Y_train_ptr_client[0])
        self.n_samples_test = len(self.Y_test_ptr_client[0])
